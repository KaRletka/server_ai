...
def _handle_report(req: ReportRequest, report_type: str) -> ReportResponse:
    ...
    today = date.today()
    saved_path = storage_service.save_report(base_info.base_id, report_type, report_content, today)

    logger.info(f"[{base_info.base_id}] {report_type}-отчёт сохранён: {saved_path}")
